...
def get_storage_hv_dose(sql_engine, date, verbose=False):
	"""
	Read storage_hv_dose table and return data for that query_time
	"""
	query = f"SELECT * FROM storage_hv_dose WHERE DATE(time) = \"{date}\";"
...
def get_storage_pressure(sql_engine, date, verbose=False):
	"""
	Read storage_pressure table and return data for that query_time
	"""
	query = f"SELECT * FROM storage_pressure WHERE DATE(time) = \"{date}\";"
	df = pd.read_sql(query, sql_engine)
...
def get_storage_refDet(sql_engine, date, verbose=False):
	"""
	Read storage_ref_det table and return data for that query_time
	"""
	query = f"SELECT * FROM storage_ref_det WHERE DATE(time) = \"{date}\";"
	df = pd.read_sql(query, sql_engine)
...
def get_storage_d2flow(sql_engine, date, verbose=False):
	"""
	Read storage_d2flow table and return data for that query_time
	"""
	query = f"SELECT * FROM storage_d2flow WHERE DATE(time) = \"{date}\";"
	df = pd.read_sql(query, sql_engine)
...
def get_storage_mw(sql_engine, date, verbose=False):
	"""
	Read storage_d2flow table and return data for that query_time
	"""
	query = f"SELECT * FROM storage_mw WHERE DATE(time) = \"{date}\";"
	df = pd.read_sql(query, sql_engine)
...
@app.callback(
	...
	[State('dropdown_dates', 'value')])
def plot_hv(n_clicks, date):
	if n_clicks is None:
		raise dash.exceptions.PreventUpdate

	# load data from database
	df_hv_dose = get_storage_hv_dose(sql_engine, date, verbose=False)
...
def plot_hv(n_clicks, date):
	...
	# load data from database
	df_hv_dose = get_storage_hv_dose(sql_engine, date, verbose=False)
	df_refDet = get_storage_refDet(sql_engine, date,verbose=False)
...
@app.callback(
	...
	[Input('btn_storage_load_and_plot', 'n_clicks')],
	[State('dropdown_dates', 'value')])
def plot_pressure(n_clicks, date):
	if n_clicks is None:
		raise dash.exceptions.PreventUpdate

	# load data from database
	df_pressure = get_storage_pressure(sql_engine, date,verbose=False)
...
@app.callback(
	...
	[Input('btn_storage_load_and_plot', 'n_clicks')],
	[State('dropdown_dates', 'value')])
def plot_d2flow(n_clicks, date):
	if n_clicks is None:
		raise dash.exceptions.PreventUpdate

	# load data from database
	df_d2flow = get_storage_d2flow(sql_engine, date,verbose=False)
...
@app.callback(
	...
	[Input('btn_storage_load_and_plot', 'n_clicks')],
	[State('dropdown_dates', 'value')])
def plot_mw_power(n_clicks, date):
	if n_clicks is None:
		raise dash.exceptions.PreventUpdate

	# load data from database
	df_mw = get_storage_mw(sql_engine, date,verbose=False)
...
@app.callback(
	...
	[Input('btn_storage_load_and_plot', 'n_clicks')],
	[State('dropdown_dates', 'value')])
def plot_mw_freq(n_clicks, date):
	if n_clicks is None:
		raise dash.exceptions.PreventUpdate
...
@app.callback(
	[
		Output("download_link_hv_dose_storage", "href"),
		Output("download_link_refDet_storage", "href"),
		Output("download_link_pressure_storage", "href"),
		Output("download_link_d2flow_storage", "href"),
		Output("download_link_mw_storage", "href"),
	],
	[Input('btn_storage_load_and_plot', 'n_clicks')],
	[State('dropdown_dates', 'value')])
def plot_hv(n_clicks, date):
